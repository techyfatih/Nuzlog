...
class EditModal extends React.Component {
  ...
  updatePokemon(change) {
    this.setState(prevState => {
      ...
      for (let i in change) {
        if (equals(change[i], this.props.pokemon[i])) delete change[i];
      }
      return { change };
    });
  }
      ...
      >
          ...
          <Tabs defaultActiveKey={1} id="edit-tabs" animation={false}>
            ...
            <Tab eventKey={5} title="Evolve/Ability">
              <Panel bsStyle="success">
                <Row className="row-no-padding">
                  <Col xs={6}>
                    <p>
                      <em>Previously:</em> {prev.species}
                    </p>
                  </Col>
                  <Col xs={6}>
                    <p>
                      <em>Previous Ability: </em>
                      {prev.ability}
                    </p>
                  </Col>
                </Row>
            ...
            <Tab eventKey={6} title="Other">
              <Panel>
                ...
                <p>
                  <em>Previous Location: </em>
                  {prev.method} {prev.location}
                </p>
        ...
        <Modal.Footer>
          <Button
            ...
            disabled={!species || !location || Object.keys(change).length == 0}
          >
            Edit Pokémon
          </Button>
        </Modal.Footer>
