    ...
    try {
      ...
      for (let i = 3; i < save.length; i++) {
        ...
        if (isNaN(time)) {
          ...
          if (hour && ampm[1].toLowerCase() == "pm") hour += 12;
        ...
        if (type == "pokemon") {
          ...
          log.entry = {
            index: party[index],
            cause: entry
              .substring(entry.indexOf("Cause of death: ") + 15)
              .trim()
          };
          party = [...party.slice(0, index), ...party.slice(index + 1)];
        } else {
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Modal.Body>
          <p>
            If you need to import from Nuzlog v1, paste your save here and click
            import.
          </p>
          <p>
            Note that this is only for backwards compatibility; please use the
            new JSON format for 2.0 if you can.
          </p>
          <FormControl
